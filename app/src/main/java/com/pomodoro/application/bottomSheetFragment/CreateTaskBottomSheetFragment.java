...
public class CreateTaskBottomSheetFragment extends BottomSheetDialogFragment {
    ...
    int minHour = currentTime.getHours();
    int minMin = currentTime.getMinutes();
    Unbinder unbinder;
    ...
    public boolean validateFields() {
        String inpTime[] = taskTime.getText().toString().split(":");
        int inpHour = Integer.parseInt(inpTime[0]);
        int inpMin = Integer.parseInt(inpTime[1]);
        ...
        else if(inpHour < minHour){
            if(inpMin < minMin) {
                Toast.makeText(activity, "Time must be beyond current time", Toast.LENGTH_SHORT).show();
                return false;
            }
            else{
                Toast.makeText(activity, "Time must be beyond current time", Toast.LENGTH_SHORT).show();
        ...
        else if(inpHour == minHour && inpMin < minMin){
            Toast.makeText(activity, "Time must be beyond current time", Toast.LENGTH_SHORT).show();
            return false;
        }
        else {
    ...
    public void createAnAlarm() {
        try {
            ...
            cal.set(Calendar.DATE, Integer.parseInt(dd)-1);
